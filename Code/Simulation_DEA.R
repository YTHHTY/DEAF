...
for (ED in ED_list){
  ...
  for (i in 1:N_test){
    ...
    result <- model_basic(data_example,
                          orientation = "io",
                          rts = "vrs");
    Effi0[i,] = unname(efficiencies(result));
    ...
    result <- model_basic(data_example,
                          orientation = "io",
                          rts = "vrs");
    Effi[i,] = unname(efficiencies(result));
  }
  write.table(Effi0, file=paste0('Result/DEA0_ReducedForm_500_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
  write.table(Effi, file=paste0('Result/DEA_ReducedForm_500_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
}
