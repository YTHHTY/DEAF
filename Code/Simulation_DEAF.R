...
source('Fun_ReducedForm.R');

Time_start <- Sys.time();    # Record time
N_test <- 100;               # Number of tests
N_iteration <- 100;          # Bootstrap iterations/The number of trees

DMU = 50;          # Number of DMUs
Num_X = 8;         # Number of Inputs: 4, 6, 8, 10, 30
...
for (ED in ED_list){
  ...
  for (i in 1:N_test){
    ...
    Effi0[i,] = DEAF(X,Y,N_iteration,1,1);
    
    ## Obtaining Pseudo Efficiency on Observational Data
    X= data[1:Num_X,];
    Y= data[(Num_X+1):(Num_X+Num_Y),]; 
    Effi[i,] = DEAF(X,Y,N_iteration,1,1);
  }
  write.table(Effi0, file=paste0('Result/DEAF0_ReducedForm_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
  write.table(Effi, file=paste0('Result/DEAF_ReducedForm_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
}
