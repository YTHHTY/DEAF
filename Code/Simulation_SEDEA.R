...
for (ED in ED_list){
  ...
  write.table(Effi0, file=paste0('Result/SEDEA0_Translog_500_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
  write.table(Effi, file=paste0('Result/SEDEA_Translog_500_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
}
