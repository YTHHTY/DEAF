...
N_iteration <- 100;          # Bootstrap iterations

DMU = 50;          # Number of DMUs
Num_X = 30;         # Number of Inputs: 4, 6, 8, 10, 30
...
for (ED in ED_list){
  ...
  for (i in 1:N_test){
    ...
    Effi0[i,] = BDEA(X,Y,N_iteration);
    
    ## Obtaining Pseudo Efficiency on Observational Data
    X= data[1:Num_X,]; 
    Y= data[(Num_X+1):(Num_X+Num_Y),]; 
    Effi[i,] = BDEA(X,Y,N_iteration);
  }
  write.table(Effi0, file=paste0('Result/BDEA0_ReducedForm_500_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
  write.table(Effi, file=paste0('Result/BDEA_ReducedForm_500_', Num_X, '_', rho, '.csv'), sep=',', append = TRUE, row.names = FALSE, col.names = FALSE);
}
