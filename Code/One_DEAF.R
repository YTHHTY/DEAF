...
data0 <- read.csv(paste('Data/Data_Io_5_9.csv', sep = ""),header = T);
...
for (i in 1:N_test){
  ...
  Effi[i,] = DEAF_BCC(X,Y,N_iteration,1,1);
}
write.table(Effi, file=paste0('Result/实证/DEAF_Io_5_9.csv'), sep=',', row.names = FALSE, col.names = FALSE);
